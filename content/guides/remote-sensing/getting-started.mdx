...
This will give you access to our beta Titiler service (v2.x) with the following endpoints:

- `/cog/tiles/{tileMatrixSetId}/{z}/{x}/{y}`
- `/cog/WebMercatorQuad/tilejson.json` — map extent in WGS84 (replaces removed `/cog/bounds`)
- `/cog/info`, `/cog/statistics` — metadata and band stats
	...
	<body>
		...
		<script>
			...
			async function initMap() {
				try {
					// TiTiler 2.x: use tilejson for WGS84 bounds (MapLibre expects lng/lat)
					const tilejsonResponse = await fetch(
						`https://your-project.geobase.app/titiler/v1/cog/WebMercatorQuad/tilejson.json?url=${encodeURIComponent(cogUrl)}&apikey=your-api-key`,
					);
					...
					map.on("load", () => {
						map.addSource("cog-layer", {
							type: "raster",
							tiles: [
								`https://your-project.geobase.app/titiler/v1/cog/tiles/WebMercatorQuad/{z}/{x}/{y}?url=${encodeURIComponent(cogUrl)}&apikey=your-api-key`,
							],
							tileSize: 256,
						});
