...
A tile server function is a four-step process:
...
| 4     | Generate the vector tile          | `vector_tile = GENERATE_VECTOR_TILE(filtered_data)`    | ```SELECT ST_AsMVT(mvtgeom, 'default') FROM mvtgeom; $$ LANGUAGE 'sql'; ```                                     |

This table maps the function's high-level structure, pseudocode abstraction, and actual implementation side by side.
