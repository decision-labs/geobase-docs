...
<Callout type="info" emoji="🔗">
  All AI tasks in GeoAI.js maintain spatial context through GeoRawImage, enabling seamless integration between AI results and geospatial workflows.
</Callout>

## Error Handling

<Callout type="warning" emoji="⚠️">
  Always validate converted coordinates against image bounds to ensure they fall within the expected geographic extent.
</Callout>
...
<Callout type="info" emoji="💡">
  Following these best practices will help ensure optimal performance and reliability when working with georeferenced imagery.
</Callout>

1. **Memory Management**: Clone only when necessary due to memory implications
2. **Precision**: Understand coordinate precision limitations with pixel-based conversions
3. **CRS Awareness**: Ensure consistent coordinate reference systems across operations
4. **Image Bounds**: Verify image bounds match expected geographic extent
