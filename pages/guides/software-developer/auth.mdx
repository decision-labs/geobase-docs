...
In this guide, we'll walk you through implementing a modern authentication system using Next.js 14 and Geobase, featuring server-side session management, protected routes, and secure middleware handling.
...
[Geobase Blueprint Auth GitHub Repository](https://github.com/decision-labs/geobase-blueprint-auth)
...
```typescript:utils/geobase/client.ts
...
```typescript:utils/geobase/server.ts
...
```typescript:utils/geobase/middleware.ts
import { createServerClient } from '@supabase/ssr'
...
Create a `.env.local` file with your Geobase credentials:

```env
NEXT_PUBLIC_GEOBASE_URL=https://YOUR_PROJECT_ID.geobase.co
NEXT_PUBLIC_GEOBASE_ANON_KEY=YOUR_GEOBASE_ANON_KEY
```
